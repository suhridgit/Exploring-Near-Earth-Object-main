...
class TestWriteToCSV(unittest.TestCase):
    ...
    def setUpClass(cls, mock_file):
        ...
        with UncloseableStringIO() as buf:
            ...
            try:
                ...
                raise cls.failureException("Unexpected "
                                           "failure while writing to CSV.") \
                    from err
            else:
                # Rewind the unclosed
                # buffer to save its contents.
                buf.seek(0)
    ...
    def test_csv_data_is_well_formed(self):
        ...
        try:
            ...
            raise self.failureException("write_to_csv "
                                        "produced an invalid CSV format.") \
                from err
    ...
    def test_csv_data_has_header(self):
        try:
            ...
            raise self.failureException("Unable to sniff for headers.") \
                from err

    def test_csv_data_five_rows(self):
        # Now, we have the value in memory, and can _actually_ start testing.
        buf = io.StringIO(self.value)
...
class TestWriteToJSON(unittest.TestCase):
    ...
    def setUpClass(cls, mock_file):
        ...
        with UncloseableStringIO() as buf:
            ...
            try:
                ...
                raise cls.failureException("Unable to "
                ...
                # Rewind the unclosed buffer to
                # fetch the contents saved to "disk".
                buf.seek(0)
                cls.value = buf.getvalue()
    ...
    def test_json_data_is_well_formed(self):
        ...
        try:
            ...
            raise self.failureException("write_to_json "
                                        "produced an invalid JSON document") \
                from err
    ...
    def test_json_element_decodes_to_correct_types(self):
        ...
        try:
            ...
            self.fail("The `datetime_utc` "
                      "key isn't in YYYY-MM-DD HH:MM` format.")
